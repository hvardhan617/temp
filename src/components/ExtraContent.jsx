...
const ExtraContent = ({ featured }) => {
  ...
  return (
    <div className="flex flex-col gap-5 lg:gap-5 mt-4">
      <CheckoutCard />
      <Featured data={featured} />
      <div>
        <Accordion title="Details" type={'reviews'}>
          <div
            dangerouslySetInnerHTML={{
              __html: selectedVariant.details,
            }}
            className="styleShopifyDetails"
          />
        </Accordion>
        ...
        {selectedVariant.amazonDetails && selectedVariant.amazonDetails.bulletPoint.length > 0 && (
          <Accordion title="Overview" type={'overview'}>
            {getAccordion('overview', selectedVariant.amazonDetails.bulletPoint)}
          </Accordion>
        )}
